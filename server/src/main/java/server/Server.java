...
public class Server {
    ...
    public int run(int desiredPort) {
        ...
        Spark.post("/session", (request, response) -> {
            try{
                return new LoginHandler().loginHandler(request, response, serializer, userDataDAO, authDataDAO, userService);
            }
            catch (InvalidUsernameException e){
                response.status(401);
        ...
        Spark.delete("/session", (request, response) -> {
            try{
                return new LogoutHandler().logoutHandler(request, response, serializer, authDataDAO, userService);
            }
            catch (InvalidInputException e){
                response.status(401);
        ...
        Spark.post("/game", (request, response) -> {
            try{
                return new CreateGameHandler().createGameHandler(request, response,
                        serializer, authDataDAO, gameDataDAO, gameService);
            }
            catch (InvalidUsernameException e){
                response.status(400);
        ...
        Spark.put("/game", (request, response) -> {
            try{
                return new JoinGameHandler().joinGameHandler(request, response, serializer,
                        authDataDAO, gameDataDAO, gameService);
            }
            catch (InvalidUsernameException e){
                response.status(401);
                return serializer.toJson(Map.of("message", "Error: Invalid username"));
        ...
        Spark.get("/game", (request, response) -> {
            try{
                return new ListGamesHandler().listGamesHandler(request, response, serializer,
                        authDataDAO, gameDataDAO, gameService);
            }
            catch (InvalidUsernameException e){
                response.status(401);
        ...
        Spark.delete("/db", (request, response) ->{
            try{
                return new ClearHandler().clearHandler(request, response, serializer,
                        userDataDAO, authDataDAO, gameDataDAO, clearService);
            }
            catch (InvalidAccessException e){
                response.status(500);
